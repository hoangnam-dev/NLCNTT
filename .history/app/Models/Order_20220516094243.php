...
class Order extends Model
{
    ...
    public function scopeSearch($q) {
        if($key = request()->key) {
            $q = $q->join('khachhang','donhang.makh','=','khachhang.makh')->where('madh', 'like', '%'.$key.'%','or','khachhang.tenkh', 'like', '%'.$key.'%');
            // $q = $q->join('khachhang','donhang.makh','=','khachhang.makh')->where('khachhang.tenkh', 'like', '%'.$key.'%');
        }
        return $q;
    }
}

...
class Order extends Model
{
    ...
    public function scopeSearch($q) {
        if($key = request()->key) {
            $date = date_format(date_create(request()->key), 'Y-m-d');
            $q = Order::join('khachhang','donhang.makh','=','khachhang.makh')->where('khachhang.tenkh', 'like', '%'.$key.'%')->orWhere('donhang.madh', $key)->orWhere('donhang.ngaydh', 'like', '%'.$date.'%'); 
        }
        return $q;
    }
}

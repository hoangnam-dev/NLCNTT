...
class AdminController extends Controller
{
    ...
    public function index() {
        ...
        $this->data['listUsers'] = Admin::orderBy('manv','ASC')->with('role')->search()->paginate($this->paginateNumber);
    ...
    public function update(Request $request) {
        ...
        if(!is_null($request->adm_password)) {
            if($request->adm_password != $request->reenter_pass) {
                return redirect()->back()->with('status', 'danger')->with('message', 'Mật khẩu nhập lại không đúng');
            } elseif(is_null($request->reenter_pass)){
                return redirect()->back()->with('status', 'danger')->with('message', 'Mật khẩu nhập lại không được trống');
            }
        }
        ...
        // dd(!is_null($request->adm_password));
        if(!is_null($request->adm_password)) {
            $adminModel->password = bcrypt($request->adm_password);
        }
        $adminModel->gioitinh = $request->adm_gender;
        $adminModel->update_at = date('Y-m-d H:i:s');
        $adminModel->save();
        return redirect()->back()->with('status', 'success')->with('message', 'Cập nhật nhân viên thành công');
    }

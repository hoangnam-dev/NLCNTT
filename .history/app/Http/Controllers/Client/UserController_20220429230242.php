...
class UserController extends Controller
{
    ...
    public function handleResetPasswordForm(Request $request) {
        ...
        if($customer === null) {
            ...
            return redirect()->route('client.user.reset-password')->with('customer', $customer);
        }
    }
    public function verificationCode(Request $request) {
        $this->data['title'] = 'Nhập mã xác nhận';
        $confirm_email = true;
        return view('client.users.reset_PW.confirmVerificationCode', $this->data)->with('confirm_email', $confirm_email);
    }
    public function handelVerificationCode(Request $request) {
        ...
        if($customer->resetpass_token === $request->verification_code) {
           return redirect()->route('client.user.reset-password', ['title' => 'Đặt lại mật khẩu'])->with('customer', $customer);
        } 
        else {
            return redirect()->route('client.user.reset-password')->with('status', 'danger')->with('message', 'Mã xác nhận không đúng');
        }
    }

    public function resetPasswordFrom(Request $request) {
        $this->data['title'] = 'Đặt lại mật khẩu';
        $this->data['customer'] = Customers::where('email', $request->email_reset)->first();

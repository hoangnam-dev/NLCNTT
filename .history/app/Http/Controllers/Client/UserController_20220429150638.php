...
class UserController extends Controller
{
    ...
    public function handleResetPasswordForm(Request $request) {
        ...
        if($customer === null) {
            ...
            return redirect()->route('client.user.reset-password-form', compact('customer'))->with('customer', $customer);
        }
    }

    public function resetPasswordFrom(Request $request) {
        $this->data['title'] = 'Đặt lại mật khẩu';
        $this->data['customer'] = Customers::where('email', session('customer')->email)->first();
    ...
    public function handleResetPassword(Request $request) {
        $rules = [
            ...
            'customer_password_confirm' => 'required|same:new_password',
        ];

        $message = [
            'new_password.required' => 'Mật khẩu không được để trống',
            'customer_password_confirm.required' => 'Mật khẩu không được để trống',
            'customer_password_confirm.same' => 'Mật khẩu không trùng khớp',
        ];
        $request->validate($rules, $message);

        $customer = Customers::where('email', session('customer')->email)->first();

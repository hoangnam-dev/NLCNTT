...
class UserController extends Controller
{
    ...
    public function handleEmailReset(Request $request) {
        ...
        if($customer === null) {
            ...
            return redirect()->route('client.user.confirm-verification')->with('customer', $customer);
        }
    }
    ...
    public function handelVerificationCode(Request $request) {
        ...
        if($customer->resetpass_token === $request->verification_code) {
           return redirect()->route('client.user.reset-password')->with('customer', $customer);
            ...
            return redirect()->route('client.user.reset-password-form')->with('status', 'danger')->with('message', 'Mã xác nhận không đúng');
        }
    }

    public function resetPasswordFrom(Request $request) {
        $this->data['title'] = 'Đặt lại mật khẩu';
        $email_reset = session('customer')->email;

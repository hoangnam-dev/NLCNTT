...
class UserController extends Controller
{
    ...
    public function handleRegister(Request $request) {
        $rules = [
            ...
            'user_email' => 'required|unique:khachhang,email',
            'user_password' => 'required',
        ...
        $message = [
            ...
            'user_email.required' => 'Hãy nhập vào một Email',
        ...
        $this->confirmEmail($request->user_email, $request->user_name);
        ...
        if($userModel->save()) {
            ...
            $customer_email = $userModel->email;
            $name = $userModel->tenkh;
            $resetpass_token = Str::random($length = 5);
            Mail::send('mail.resetpass_token', compact('customer', 'resetpass_token'), function ($email) use ($customer_email,$name) {
                $email->to($customer_email,$name)
                ->subject('ABCShop - Xác nhận đăng ký tài khoản');
            });
            
            return view('client.users.confirmVerificationCode', compact('title', 'customer_email'));
        }
        else {
    ...
    // Confirm Email
    public function confirmAccount($email, $name) {
        // $resetpass_token = Str::random($length = 5);
        // Mail::send('mail.resetpass_token', compact('customer', 'resetpass_token'), function ($email) use ($email, $name) {
        //     $email->to($email, $name)
    ...
    public function verificationCode(Request $request) {
        ...
        return view('client.users.confirmVerificationCode', $this->data);
    }

      ...
      <form action="{{ route('admin.products.product-edit', ['masp'=>$product->masp]) }}" method="POST"
        ...
        <div class="form-floating mb-3">
          ...
          {{-- <div class="mb-3">
            ...
            <button type="button" class="btn btn-primary mb-3 add_attr" data-toggle="modal" data-target="#exampleModal"
          ...
          @endif
            ...
            <div class="mb-3">
              <label for="" class="form-label">{{ $attr->tentt }}</label>
              <input type="hidden" name="matt" value="{{ $attr->matt }}">
              <select class="form-control form-select d-block mb-2" name="giatri_tt">
                @foreach ($attr->hasAttributeDetail as $key => $value )
                <option value="{{ $value->giatri }}">{{ $value->giatri }}</option>
                @endforeach
              </select>
              <button type="button" class="btn btn-primary add_attr mb-3" data-toggle="modal"
                data-target="#exampleModal" data-attr_id="{{ $attr->matt }}" data-whatever="@getbootstrap">Thêm giá
                trị</button>
            </div>
            @endforeach
          </div>

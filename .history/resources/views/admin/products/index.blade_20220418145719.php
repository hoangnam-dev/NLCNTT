                          ...
                          <td>
                            <div class="row">
                              ...
                              <a href="{{ route('admin.products.product-edit', ['masp'=>$values->masp]) }}"
                                class="btn btn-success"><i class="fas fa-edit"></i>Chi tiết</a>
                              {{-- Xóa --}}
                              <form class="ml-3" action="{{ route('admin.products.product-status') }}" method="POST">
                                @csrf
                                {{-- @method('PUT') --}}
                                <input type="hidden" name="masp" value="{{ $values->masp }}">
                                @if($values->trangthai == 1)
                                <input type="hidden" name="status" value="0">
                                @else
                                <input type="hidden" name="status" value="1">
                                @endif
                                <button class="btn btn-danger" type="submit" name="prd_stt">
                                  <i class="fas fa-trash"></i>Ẩn/Hiển thị</button>
                              </form>
                            </div>
                          </td>
                        </tr>
...
<script>
  $( ".btn-danger" ).click(function() {
      if(confirm( 'Bạn có chắc muốn thay đổi sản phẩm này?' )==true) {
        return true;
      }
      return false;
    });

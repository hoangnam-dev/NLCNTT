                    ...
                    <tbody>
                      @foreach ($listUsers as $key => $values)
                        <tr>
                          ...
                          <td>{{ $values->email }}</td>
                          <td>
                              <form class="ml-3 d-flex justify-content-start" action="{{ route('admin.users.admin-set-role', ['manv'=>$values->manv]) }}"
                                method="post">
                                @csrf
                                @method('PUT')
                                @foreach ($listRole as $role)
                                <div class="form-check d-flex justify-content-start align-items-center mx-2">
                                  <input class="form-check-input" name="role" type="checkbox" value="{{ $role->maquyen }}" id="flexCheckDefault">
                                ...
                                <button class="btn btn-info">
                                  <i class="fa-regular fa-floppy-disk mx-1"></i>Lưu</button>
                              </form>
                          </td>
                          <td style="border-bottom: none;" class="align-items-center">

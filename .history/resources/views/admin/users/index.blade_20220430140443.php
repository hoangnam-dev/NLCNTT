                    ...
                    <td>
                      <form class="ml-3" action="{{ route('admin.users.admin-set-role', ['manv'=>$values->manv]) }}"
                        ...
                        <div class="d-flex justify-content-between align-items-start">
                          <div class="role">
                            @foreach ($listRole as $role)
                            <div class="form-check mx-2 d-flex justify-content-start align-items-center">
                              <input class="form-check-input" name="role" type="checkbox" value="{{ $role->maquyen }}" id="flexCheckDefault" {{ $values->hasRole($role->tenquyen) ? 'checked' : '' }}>
                              {{ dd($values->hasRole($role->tenquyen)) }}
                      ...
                      {{-- <button type="button" class="btn btn-primary set-role" data-manv={{ $values->manv }}
                        data-toggle="modal" data-url={{ route('admin.users.admin-get-role') }}
                        data-target="#exampleModal" data-token="{{ csrf_token() }}">
                        Phân quyền
                      </button> --}}
                    </td>

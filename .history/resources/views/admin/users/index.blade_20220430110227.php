              ...
              <table id="example2" class="table table-bordered table-hover dataTable dtr-inline">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    {{--
                      <form class="ml-3" action="{{ route('admin.users.admin-set-role', ['manv'=>$values->manv]) }}"
                        ...
                        <div class="d-flex justify-content-between align-items-start">
                          <select class="form-control form-select d-block mx-2" name="role">
                            @foreach ($listRole as $key => $role)
                            <option value="{{ $role->maquyen }}" {{ $role->maquyen == $values->role->maquyen }}>{{ $role->tenquyen }}</option>
                            @endforeach
                          </select>
                    ...
                    --}}
                    <button type="button" class="btn btn-primary set-role" data-manv={{ $values->manv }} data-toggle="modal" data-url={{ route('admin.users.admin-set-role') }} data-target="#exampleModal">
                      Phân quyền
                    </button>
                    </td>

        ...
        <div class="ml-3">
            ...
            <form action="{{ route('admin.comments.comment-detail', ['mabl'=>$comment->mabl]) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <div class="images">
                        <img src="{{ asset('assets/uploads/products').'/'.$comment->hasProduct->avatar }}" class="img-fluid"
                            alt="{{ $comment->hasProduct->avatar }}">
                    </div>
                </div>
                <div class="mb-3">
                ...
                <div class="form-floating mb-3">
                    ...
                    <textarea name="reply" class="form-control" cols="30"
                        rows="10">{{ $comment->hasReplyComment->noidung }}</textarea>
                </div>
                <!-- /.col -->
                <div class="col-2 mb-3">
                    <form class="ml-3" action="{{ route('admin.comments.edit-reply', ['mabl'=>$comment->mabl]) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit" name="prd_edit_submit" class="btn btn-primary btn-block">Sửa phản hồi</button>
                      </form>
                </div>
                <form class="ml-3" action="{{ route('admin.comments.detroy-reply', ['mabl'=>$comment->mabl]) }}" method="post">
                    @csrf
                    @method('DELETE')
                    <button class="btn btn-danger">Xóa phản hồi</button>
                  </form>
                @else
                <div class="form-floating mb-3">
                    <label for="floatingTextarea2">Phản hồi</label>

...
<div class="row">
    <div class="col-12">
        <div class="ml-3">
            ...
            <form action="{{ route('admin.comments.edit-reply', ['mabl'=>$comment->mabl]) }}" method="POST">
                @csrf
                {{-- @method('PUT') --}}
                ...
                <div class="col-2 mb-3">
                    <a href="{{ route('admin.comments.destroy-reply', ['mabl'=>$comment->mabl]) }}" class="btn btn-primary btn-block">Sửa phản hồi</a>
                </div>
            </form>
            <div class="col-2 mb-3">
                <a href="{{ route('admin.comments.destroy-reply', ['mabl'=>$comment->hasReplyComment->mabl]) }}"
                    class="btn btn-danger btn-block">Xóa phản hồi</a>
            </div>
            @else
            <form action="{{ route('admin.comments.comment-detail', ['mabl'=>$comment->mabl]) }}" method="POST">
                ...
                <div class="row d-flex justify-content-between">
                    <div class="col-2 mb-3">
                        {{-- <form class="ml-3"
                            action="{{ route('admin.comments.edit-reply', ['mabl'=>$comment->mabl]) }}" method="post">
                            @csrf
                            @method('POST')
                        </form> --}}
                        @method('POST')
                        <button type="submit" name="prd_edit_submit" class="btn btn-primary btn-block">Sửa phản
                            hồi</button>
                    </div>
                    <div class="col-2 mb-3">
                        <a href="{{ route('admin.comments.destroy-reply', ['mabl'=>$comment->mabl]) }}"
                            class="btn btn-danger btn-block">Xóa phản hồi</a>
                    </div>
                </div>
                <div class="form-floating mb-3">

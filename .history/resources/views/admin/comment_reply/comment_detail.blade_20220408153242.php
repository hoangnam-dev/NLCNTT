    ...
    <div class="col-12">
        <div class="ml-3">
            ...
            <form action="{{ route('admin.comments.comment-detail', ['mabl'=>$comment->mabl]) }}" method="POST">
                ...
                <div class="col-2 mb-3">
                    <form class="ml-3" action="{{ route('admin.comments.destroy-reply', ['mabl'=>$comment->mabl]) }}"
                        method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit" name="prd_edit_submit" class="btn btn-primary btn-block">Xóa phản
                            hồi</button>
                    </form>
                </div>
                @else

        ...
        <div class="col-md-3 pe-5">
                ...
                <ul class="list-unstyled ps-0">
                    ...
                    <li class="mb-1">
                        <a href="{{ route('client.catergory') }}" class="user-link">
                            <button type="button" class="category_link cacategory-link_active">
                                Tất cả
                            </button>
                        </a>
                    </li>
                    ...
                    <li class="mb-1">
                        {{-- <a href="{{ route('client.productsbycatergory', ['danh_muc'=>$category->madm]) }}"
                            class="user-link"> --}}
                            {{-- <div class="btn"></div> --}}
                            <button type="button" class="category_link" data-id="{{ $category->madm }}" data-url="{{ route('client.productsbycatergory', ['danh_muc'=>$category->madm]) }}">
                                {{ $category->tendm }}
                            </button>
                        </a>
                    </li>
            ...
            <div class="flex-shrink-0 p-3" style="width: 280px;">
                ...
                {{-- <form action=""> --}}
                    ...
                    <button id="sort-price" class="btn btn-warning">
                        Lọc
                    </button>
                {{-- </form> --}}
            </div>
...
$('.category_link').on('click', function () {
    ...
    var id = $(this).data('id');
    $('.category_link').removeClass('cacategory-link_active');
    $(this).addClass('cacategory-link_active');
    $.ajax({
        url: url,
        type: "GET",
        data: {
...
$('#sort-price').on('click', function () {
    var category_id = $('.cacategory-link_active').data('id');
    alert(category_id);
});
</script>
@endsection
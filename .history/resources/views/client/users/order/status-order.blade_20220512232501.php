...
<div class="row  mb-4">
    <div class="col-md-12">
        <div class="order-title">Mã đơn hàng: <strong>{{ $order->madh }}</strong> | <span class="text-danger">{{ $status }} {{ $order->trangthai==0?'active':'' }}</span></div>
    </div>
    <div class="order-step col-sm-12 col-md-8 mx-auto">
        ...
        <div class="order_step {{ $order->trangthai==0?'acitve':'' }} {{ $order->trangthai>0?'active complete':'' }} complete">
            <div class="order_step-status">
                <i class="order_step-status_icon fa-solid fa-file-pen"></i>
            </div>
            <div class="order_step-title">Chờ xử lý</div>
        </div>
        <div class="order_step {{ $order->trangthai==1?'active':'' }} {{ $order->trangthai>1?'active complete':'' }}">
            <div class="order_step-status">
                <i class="order_step-status_icon fa-solid fa-truck-fast"></i>
            </div>
            <div class="order_step-title">Đang giao hàng</div>
        </div>
        <div class="order_step {{ $order->trangthai==2?'active':'' }}">
            <div class="order_step-status">
                <i class="order_step-status_icon fa-solid fa-clipboard-check"></i>
            </div>

...
<form action="{{ route('client.user.confirm-verification') }}" method="POST">
    ...
    <div class="resend">Không nhận được mã? <span class="resend-link text-danger font-weight-bold" data-url="{{ route('client.user.resend-verify-code') }}" data-token="{{ csrf_token() }}">Gửi lại</span><div class="message mx-2"></div></div>
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.status == 'success'){
                        ...
                        $('.message').html('<span class="alert alert-success w-100">Mã xác nhận đã được gửi</span>');
                    }else{

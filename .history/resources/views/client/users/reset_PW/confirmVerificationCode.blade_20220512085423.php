...
<form action="{{ route('client.user.confirm-verification') }}" method="POST">
    ...
    <div class="resend">Không nhận được mã? <span class="resend-link text-danger font-weight-bold" data-url="{{ route('client.user.resend-verify-code') }}" data-token="{{ csrf_token() }}">Gửi lại</span></div>
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.status == 'success'){
                        ...
                        var htmlString = '<span class="alert alert-success">Mã xác nhận đã được gửi</span>';
                        $('.message').html(htmlString);
                    }else{
                        alert('Gửi mã xác nhận thất bại.');
                        var htmlString = '<span class="alert alert-success">Gửi mã xác nhận thất bại</span>';
                        $('.message').html(htmlString)
                    }
                }
            }); 
